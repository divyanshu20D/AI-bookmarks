...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const [editingBookmark, setEditingBookmark] =
    useState<FrontendBookmark | null>(null);
  ...
  /*
  // Debounced search-on-type logic (commented out for future use)
  useEffect(() => {
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="relative flex-1 flex">
              ...
              <Input
                ...
                className="pl-10 h-12 text-base flex-1"
                disabled={isSearching}
              />
              <Button
                onClick={handleSearch}
                className="ml-2 h-12 px-6 flex items-center space-x-2"
                disabled={isSearching}
              >
                <span>Search</span>
              </Button>
          ...
          {searchQuery && (
            <p className="text-sm text-gray-600">
              {isSearching
                ? "Searching..."
                : `Found ${filteredBookmarks.length} result${
